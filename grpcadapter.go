...
func (s *GrpcAdapter) sendStatMessage(r *authorization.HandleAuthorizationRequest) {
	...
	sm := StatMessage{Key: types.NamespacedName{Namespace: namespace, Name: strings.TrimSuffix(service, "-priv")}}
	...
	now := time.Now()
	sm.Stat.Time = &now
	err := s.statSink.Send(&sm)
	if err != nil {
		log.Errorf("Error while sending stat %v\n", err)
	}
	log.Debugf("Stat data sent %#v\n", sm)
}
